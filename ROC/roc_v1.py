...
date, value, roc_viv, roc_sfty = text_file.readline().strip('\n').split('\t')
top = int(sys.argv[2])
...
while(text_file):
    ...
    if((((float(roc_viv)*100) <= bottom) and (wallet > 0)) or ((float(roc_sfty)*100) <= bottom) and (wallet > 0)): #COMPRA
    ...
    elif((((float(roc_viv)*100) >= top) and (stocks > 0)) or ((float(roc_sfty)*100) >= top) and (stocks > 0)): #VENDA
    ...
    try:
        date, value, roc_viv, roc_sfty = text_file.readline().split('\t')
    except:
        if(stocks > 0):
            wallet = float(value)*stocks
            stocks = 0
