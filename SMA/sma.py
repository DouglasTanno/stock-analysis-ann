...
date, value, sma5, sma10, sma20 = text_file.readline().strip('\n').split('\t')
...
while(text_file):
    ...
    if((float(sma5) > float(sma10)) and (float(sma5) > float(sma20)) and (wallet > 0)): #COMPRA
    ...
    if((float(sma5) < float(sma10)) and (float(sma5) < float(sma20)) and (stocks > 0)): #VENDA
        ...
        n = n + 1
    try:
        date, value, sma5, sma10, sma20 = text_file.readline().split('\t')
    except:
        if(stocks > 0):
            wallet = float(value)*stocks
            stocks = 0
