...
date, value, rsi = text_file.readline().strip('\n').split('\t')
top = int(sys.argv[2])
...
while(text_file):
    ...
    if((float(rsi) <= bottom) and (wallet > 0)): #COMPRA
    ...
    if((float(rsi) >= top) and (stocks > 0)): #VENDA
    ...
    try:
        date, value, rsi = text_file.readline().split('\t')
    except:
        if(stocks > 0):
            wallet = float(value)*stocks
            stocks = 0
